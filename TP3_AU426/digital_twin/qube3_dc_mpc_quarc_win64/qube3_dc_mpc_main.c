...
typedef RT_MODEL_qube3_dc_mpc_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * qube3_dc_mpc(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(qube3_dc_mpc_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(qube3_dc_mpc_DW.HILReadTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://qube3_dc_mpc:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: qube3_dc_mpc -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(qube3_dc_mpc_DW.HILReadTimebase_Task, (t_clock)
        qube3_dc_mpc_P.HILReadTimebase_Clock, 50.0, -1)
        ;
      if (result == 0) {
